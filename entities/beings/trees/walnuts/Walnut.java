package entities.beings.trees.walnuts;
...
public class Walnut extends Tree {
    ...
    public Walnut(Handler handler, int gridX, int gridY) {
        super(handler, gridX, gridY);
        burningRate = 1;
    }

    public Walnut(Handler handler, int gridX, int gridY, int health, boolean onFire, boolean burned, double burningTime) {
        super(handler, gridX, gridY);
    ...
    public BufferedImage getSprite() {
        if (!burned) {
            return Assets.assets.get(Assets.Walnut);
        } else {
