package entities.beings.trees.mangos;
...
public class Mango extends Tree {
    ...
    public Mango(Handler handler, int gridX, int gridY) {
        super(handler, gridX, gridY);
        burningRate = 1;
    }

    public Mango(Handler handler, int gridX, int gridY, int health, boolean onFire, boolean burned, double burningTime) {
        super(handler, gridX, gridY);
    ...
    public BufferedImage getSprite() {
        if (!burned) {
            return Assets.assets.get(Assets.Mango);
        } else {
